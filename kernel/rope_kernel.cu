...
__global__ void apply_rope_inplace_half_kernel(
    uint16_t* tensor_raw,
    const size_t* positions,
    int num_tokens,
    int num_heads,
    ...
    __half* tensor = reinterpret_cast<__half*>(tensor_raw);
    const float x0 = __half2float(tensor[even_idx]);
    const float x1 = __half2float(tensor[odd_idx]);

    tensor[even_idx] = __float2half(x0 * c - x1 * s);
